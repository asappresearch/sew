...
class FeatMLPWav2Vec2Config(FairseqDataclass):
    ...
    fbank_dim: int = II("task.fbank_dim")
    mfcc_dim: int = II("task.mfcc_dim")
...
class FeatMLPWav2Vec2Model(BaseFairseqModel):
    ...
    def forward(self, source, padding_mask=None, audio_feats=None, mask=True, features_only=False,
                embed_inputs=False, before_context=False):
    ...
    def extract_features(self, source, padding_mask, audio_feats=None, mask=False, embed_inputs=False):
        res = self.forward(source, padding_mask, audio_feats=audio_feats, mask=mask, features_only=True, embed_inputs=embed_inputs)
